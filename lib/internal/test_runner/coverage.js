...
const { getLazy, setupCoverageHooks } = require('internal/util');
const { tmpdir } = require('os');
const { join, resolve, relative } = require('path');
const { fileURLToPath, URL } = require('internal/url');
const { kMappings, SourceMap } = require('internal/source_map/source_map');
// Only loaded when statement coverage is enabled.
const lazyAcornParser = getLazy(() =>
  require('internal/deps/acorn/acorn/dist/acorn').Parser);
const lazyAcornWalkSimple = getLazy(() =>
...
class TestCoverage {
  ...
  getStatements(fileUrl, source) {
    ...
    const AcornParser = lazyAcornParser();
    const acornWalkSimple = lazyAcornWalkSimple();
